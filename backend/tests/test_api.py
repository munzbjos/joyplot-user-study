...
async def test_continuous_zoom_metrics_are_stored_and_exported(client):
    _,h=await new_ready(client); trial=(await client.post("/api/session/start",headers=h)).json()["trials"][0]
    payload={"selected_answer":trial["options"][0]["id"],"rt_selection_ms":120,"rt_submit_ms":640,"answer_changes":0,"zoom_used":True,"zoom_count":2,"zoom_duration_ms":321.5,"max_zoom_pct":219.7,"trial_restarted":False,"restart_count":0}
