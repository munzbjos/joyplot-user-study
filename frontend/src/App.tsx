...
export function App() {
  ...
  return <DesktopGate><main className={`shell ${usesMapCanvas ? 'map-shell' : ''} ${view === 'trial' ? 'trial-shell' : ''}`}>
    ...
    {view === 'thank-you' && <section className="status"><h1>Thank You!</h1><p>Thank you for taking part in this study.</p><p>Your responses have been recorded successfully.</p><nav className="contact-links" aria-label="Researcher contact and publication"><a href="https://www.linkedin.com/in/josef-m%C3%BCnzberger-a71a29204/" target="_blank" rel="noopener noreferrer" aria-label="Josef Münzberger on LinkedIn"><ContactIcon kind="linkedin" />LinkedIn</a><a href="mailto:josef.munzberger@fsv.cvut.cz" aria-label="Email Josef Münzberger"><ContactIcon kind="mail" />josef.munzberger@fsv.cvut.cz</a><a href="https://doi.org/10.1080/00087041.2026.2715285" target="_blank" rel="noopener noreferrer" aria-label="Bivariate Joy Plot article"><ContactIcon kind="article" />Bivariate Joy Plot article</a></nav></section>}
...
export function Training({ index, onContinue }: { index: number; onContinue: () => void }) {
  ...
  return <section><h1 className="practice-heading"><span>{item.headingPrefix}</span> <strong>{item.methodLabel}</strong></h1><ImageViewer cropChoroplethFrame={item.method === 'CH'} className="practice-viewer" src={item.assetUrl} alt={`${item.methodLabel} training stimulus`} />
...
function ContactIcon({ kind }: { kind: 'linkedin' | 'mail' | 'article' }) {
  if (kind === 'linkedin') return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M6.5 8.3H3.2V21h3.3V8.3ZM4.8 3A1.9 1.9 0 1 0 4.8 6.8 1.9 1.9 0 0 0 4.8 3ZM21 13.7c0-3.8-2-5.6-4.8-5.6a4.2 4.2 0 0 0-3.8 2.1V8.3H9.1V21h3.3v-6.3c0-1.7.3-3.3 2.4-3.3 2 0 2.1 1.9 2.1 3.4V21H21v-7.3Z" /></svg>
  if (kind === 'mail') return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M3 5h18v14H3V5Zm2 2v.5l7 5 7-5V7H5Zm14 10V10l-7 5-7-5v7h14Z" /></svg>
  return <svg aria-hidden="true" viewBox="0 0 24 24"><path d="M5 3h10l4 4v14H5V3Zm2 2v14h10V9h-4V5H7Zm2 7h6v2H9v-2Zm0 4h6v2H9v-2Z" /></svg>
}
