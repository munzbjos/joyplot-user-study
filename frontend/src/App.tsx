...
function JoyInstructions({ onBack, onContinue }: { onBack: () => void; onContinue: () => void }) { return <section><h1>Bivariate Joy Plot</h1><p>A bivariate joy plot represents spatial values using a series of profiles.</p><p><strong>Variable A</strong> and <strong>Variable B</strong> are shown as two overlaid sets of ridges. <strong>The legend identifies the variables by colour: Variable A is blue and Variable B is red.</strong></p><p>The <strong>height of a ridge represents the value of the variable at that location:</strong></p><p><strong>Higher ridge = higher value.</strong></p><p>To compare values, look at the relative heights of the corresponding ridges at the location of interest.</p><ImageViewer className="instruction-viewer" src="/training/T0_J.png" alt="Bivariate Joy Plot training map with legend" /><div className="button-row"><button className="secondary" onClick={onBack}>Back</button><button onClick={onContinue}>Continue</button></div></section> }

function ChoroplethInstructions({ onBack, onContinue }: { onBack: () => void; onContinue: () => void }) { return <section><h1>Bivariate Choropleth Map</h1><p>A bivariate choropleth map represents <strong>Variable A</strong> and <strong>Variable B</strong> simultaneously using colour.</p><p>Each map cell belongs to one of nine colour classes representing a combination of values of the two variables.</p><p>Use the <strong>3 × 3 legend</strong> to interpret the colour of a cell:</p><ul><li>one direction of the legend represents <strong>Variable A</strong>, from low to high;</li><li>the other direction represents <strong>Variable B</strong>, from low to high.</li></ul><p>To identify the values at a location, match the colour of the corresponding map cell to the legend.</p><ImageViewer cropChoroplethFrame className="instruction-viewer" src="/training/T0_CH.png" alt="Bivariate choropleth training map with 3 × 3 legend" /><div className="button-row"><button className="secondary" onClick={onBack}>Back</button><button onClick={onContinue}>Continue</button></div></section> }
